...
function love.load()
    ...
    player1 = Paddle(10,30,5,20,paddle1AI,"vert","1")
    player2 = Paddle(VIRTUAL_WIDTH-10,VIRTUAL_HEIGHT-50,5,20,paddle2AI,"vert","2")
    player3 = Paddle(30,10,20,5,paddle3AI,"hor","3")
    player4 = Paddle(VIRTUAL_WIDTH-50,VIRTUAL_HEIGHT-10,20,5,paddle4AI,"hor","4")
...
function love.update(dt)
    ...
    --player 2
    if love.keyboard.isDown('[') and not player2.ele then
        player2.dy = -PADDLE_SPEED
    elseif love.keyboard.isDown("'") and not player2.ele then
        player2.dy = PADDLE_SPEED
    else
        player2.dy = 0
    end
...
function love.draw()
    ...
    love.graphics.printf("FPS: "..love.timer.getFPS(),0,VIRTUAL_HEIGHT-10, VIRTUAL_WIDTH, 'left')
    love.graphics.printf(mode,0,VIRTUAL_HEIGHT/2-10, VIRTUAL_WIDTH, 'center')
    if gameState == "start" then
        text = loadingAnimation()
        love.graphics.printf("Hit space to begin"..text,0,VIRTUAL_HEIGHT/2+40, VIRTUAL_WIDTH, 'center')
        if winner == "player1" then
            love.graphics.setColor(220,20,60)
            love.graphics.printf("Red has won! Congrats!",0,VIRTUAL_HEIGHT/2-40, VIRTUAL_WIDTH, 'center')
        elseif winner == "player2" then
            love.graphics.setColor(0,191,255)
            love.graphics.printf("Blue has won! Congrats!",0,VIRTUAL_HEIGHT/2-40, VIRTUAL_WIDTH, 'center')
        elseif winner == "player3" then
            love.graphics.setColor(50,205,50)
            love.graphics.printf("Green has won! Congrats!",0,VIRTUAL_HEIGHT/2-40, VIRTUAL_WIDTH, 'center')
        elseif winner == "player4" then
            love.graphics.setColor(255,215,0)
            love.graphics.printf("Yellow has won! Congrats!",0,VIRTUAL_HEIGHT/2-40, VIRTUAL_WIDTH, 'center')
        end
    end
    -- Print Score
    ...
    love.graphics.setColor(220,20,60)
    if player1.ai then
        love.graphics.printf(player1.score.."/"..START_SCORE.."\n(AI)",-VIRTUAL_WIDTH/2+60,VIRTUAL_HEIGHT/2,VIRTUAL_WIDTH,"center")
    else
        love.graphics.printf(player1.score.."/"..START_SCORE.."\n(W + S)",-VIRTUAL_WIDTH/2+60,VIRTUAL_HEIGHT/2,VIRTUAL_WIDTH,"center")
    end
    player1:render()
    love.graphics.setColor(0,191,255)
    if player2.ai then
        love.graphics.printf(player2.score.."/"..START_SCORE.."\n(AI)",VIRTUAL_WIDTH/2-50,VIRTUAL_HEIGHT/2,VIRTUAL_WIDTH,"center")
    else
        love.graphics.printf(player2.score.."/"..START_SCORE.."\n([ + ')",VIRTUAL_WIDTH/2-50,VIRTUAL_HEIGHT/2,VIRTUAL_WIDTH,"center")
    end
    player2:render()
    love.graphics.setColor(50,205,50)
    if player3.ai then
        love.graphics.printf(player3.score.."/"..START_SCORE.."\n(AI)",0,30,VIRTUAL_WIDTH,"center")
    else
        love.graphics.printf(player3.score.."/"..START_SCORE.."\n(T + G)",0,30,VIRTUAL_WIDTH,"center")
    end
    player3:render()
    love.graphics.setColor(255,215,0)
    if player4.ai then
        love.graphics.printf(player4.score.."/"..START_SCORE.."\n(AI)",0,VIRTUAL_HEIGHT-70,VIRTUAL_WIDTH,"center")
    else
        love.graphics.printf(player4.score.."/"..START_SCORE.."\n(U + J)",0,VIRTUAL_HEIGHT-70,VIRTUAL_WIDTH,"center")
    end
    player4:render()
