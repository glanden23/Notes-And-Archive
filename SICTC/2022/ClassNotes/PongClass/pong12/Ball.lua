...
function Ball:update(dt)
    ...
    if ballType == "Free" then
        move = love.math.random(2)
        if love.math.random(2) == 1 then
            self.x = self.x + love.math.random(-1, 1)
        else
            self.y = self.y + love.math.random(-1, 1)
        end
    end
end
...
function Ball:collide()
    ...
    elseif ballType == "Bouncer" then
        self.DY = love.math.random(2) < 2 and 150 or -150
    else
        self.DY = self.DY + love.math.random(-10, 10)
    end
end
